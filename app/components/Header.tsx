...
const Header = ({ mainRef }: HeaderProps) => {
    ...
    const highlightRef = useRef<HTMLDivElement>(null); // Ref for the highlight element
    const [initialHighlightOffset, setInitialHighlightOffset] = useState(40);
    ...
    const highlightSection = useCallback((target: Element) => {
        if (target && highlightRef.current) {
            ...
            const newStyle = {
                width: `${width}px`,
                left: `${left}px`,
                top: `${top + 25 + initialHighlightOffset}px`
            };

            // Directly update the highlight element's style
            Object.assign(highlightRef.current.style, newStyle);

            setInitialHighlightOffset(0);
        }
    }, [initialHighlightOffset]);
        ...
        <motion.header initial={initialFadeDown} animate={fadeIn(0.2)} ref={headerRef} className="py-4 sm:py-10 sticky top-0 backdrop-blur-lg z-20 header">
            <div className="custom-container px-5 mx-auto">
                <div className="logo-menu-wrap flex justify-between items-center">
                    ...
                    <div ref={highlightRef} className="highlight absolute h-1 bg-white transition-all rounded-full"></div>
