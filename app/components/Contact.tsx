                    ...
                    <AnimatePresence initial={false}>
                        {emailResponse.length ?
                            ...
                            <motion.form exit={{ height: 0 }} initial={{ ...initialFadeUp, height: 'auto' }} animate={fadeIn()} onSubmit={handleSubmit} method="dialog" className='flex gap-5 lg:gap-10 flex-wrap'>
                                <motion.input initial={initialFadeUp} whileInView={fadeIn(0.7)} viewport={{ once: true, root: scrollRef }} required type="text" name="name" id="name" placeholder="Name" aria-label="Name" className='flex-1 p-4 rounded-md glass' />
                                <motion.input initial={initialFadeUp} whileInView={fadeIn(0.9)} viewport={{ once: true, root: scrollRef }} required type="email" name="email" id="email" placeholder="Email" aria-label="Email" className='flex-1 p-4 rounded-md glass' />
